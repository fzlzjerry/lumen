...
- **T4 io**: added `read_bytes`/`write_bytes` (byte arrays in `0..=255`; out-of-range element throws), `stat` (`{size, is_dir, is_file, modified}`, built via the GC-safe `make_map` ported from `datetime`), `walk` (recursive pre-order DFS, sorted per directory), and `copy`/`rename`; all reuse the module's `ValueError` I/O-error convention. Suite green at **288**.
- **T5 random**: added `uniform(a, b)` ∈ [a,b), `randrange(lo, hi)` ∈ [lo,hi) (`lo >= hi` throws), `sample(seq, k)` (k distinct, no replacement, via partial Fisher–Yates; `k < 0` or `k > len` throws), and `gauss(mu, sigma)` (Box–Muller). Factored a shared `unit_f64` helper so `random()` is byte-for-byte unchanged and all draws stay deterministic after `seed()`. Suite green at **289**; README test count refreshed (265 → 289).
