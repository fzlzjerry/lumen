...
fn hex_decode(vm: &mut Vm, a: &[Value]) -> Result<Value, Value> {
    ...
    match String::from_utf8(out) {
        ...
        Err(_) => Err(err(
            vm,
            error_kind::VALUE,
            "hex_decode: result is not valid UTF-8",
        )),
    }
}
...
fn base64_decode(vm: &mut Vm, a: &[Value]) -> Result<Value, Value> {
    ...
    if bytes.len() % 4 != 0 {
        return Err(err(
            vm,
            error_kind::VALUE,
            "base64_decode: length is not a multiple of 4",
        ));
    }
    ...
    for chunk in bytes.chunks(4) {
        ...
        for (i, &c) in chunk.iter().enumerate() {
            if c == b'=' {
                ...
                return Err(err(
                    vm,
                    error_kind::VALUE,
                    "base64_decode: data after padding",
                ));
            } else {
                match b64_val(c) {
                    Some(v) => vals[i] = v,
        ...
        if pad > 2 {
            return Err(err(
                vm,
                error_kind::VALUE,
                "base64_decode: too much padding",
            ));
        }
    ...
    match String::from_utf8(out) {
        ...
        Err(_) => Err(err(
            vm,
            error_kind::VALUE,
            "base64_decode: result is not valid UTF-8",
        )),
    }
}
