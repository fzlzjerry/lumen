...
fn publish_diagnostics(writer: &mut impl Write, uri: &str, text: &str) {
    let (_program, errors) = crate::check_source(text);
    let diagnostics: Vec<Json> = errors
        .iter()
        .map(|d| {
            ...
            Json::Obj(vec![
                ...
                ("severity".into(), Json::Num(1.0)), // Error
...
use crate::ast::{Block, ClassDecl, Function, Pattern, PatternKind, Program, Stmt};
use crate::span::Span;
...
/// Collect every statement-level declaration in the program, each tagged with
/// the scope it's visible in. (Lambdas nested inside expressions aren't walked —
/// a documented v1 limitation; top-level/function/block/loop/catch bindings are.)
...
fn goto_definition(params: &Json, docs: &HashMap<String, String>) -> Option<Json> {
    ...
    // Among same-named declarations whose scope contains the use, pick the
    // innermost (smallest enclosing range) — lexical shadowing.
    let best = defs
        .iter()
