...
fn unused_local_warns() {
    ...
    assert!(no_warnings(
        "fn f() { let x = 1; return fn() { return x; }; }"
    ));
...
fn unreachable_code_warns() {
    ...
    assert!(has_warning(
        "fn f() { throw \"e\"; print(2); }",
        "unreachable"
    ));
    assert!(has_warning(
        "while true { break; print(2); }",
        "unreachable"
    ));
    assert!(has_warning(
...
fn wrong_arity_warns() {
    ...
    assert!(no_warnings(
        "fn f(a) { return a; } fn g() { let f = 5; return f; } g();"
    ));
}
...
fn arity_skips_reassigned_global() {
    ...
    assert!(no_warnings(
        "fn f(a, b) { return a + b; } f = fn(x) { return x; }; println(f(10));"
    ));
    assert!(no_warnings(
        "fn f(a, b) { return a; } f = fn() { return 99; }; println(f());"
    ));
